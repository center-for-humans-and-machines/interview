...
async def read_messages(session_id: str):
    conversation_doc = await Conversations.find_one(Conversations.conversation_id == session_id)
    if conversation_doc == None:
        raise HTTPException(status_code=404, detail=f"Conversation not found with session_id {session_id}")
...
async def make_table_for_one_conversation(session_id):
    ...
    search_params_data = {key: search_parameters.get(key, None) for key in search_parameters.keys()}
...
async def download_csv(session_id: str, response: Response):
    ...
    response.headers["Content-Disposition"] = f"attachment; filename=conversation_session_id_{session_id}.csv"

    return response

@download_router.get("/experiment/{project_id}/{experiment_id}")
async def download_csv_experiment(project_id: str, experiment_id: str, response: Response):
    ...
    response = StreamingResponse(iter([stream.getvalue()]), media_type="text/csv")
    response.headers["Content-Disposition"] = f"attachment; filename=experiment_id_{experiment_id}.csv"

    return response

@download_router.get("/chat/{project_id}/{experiment_id}/{participant_id}")
async def download_chat_in_json(project_id: str, experiment_id: str, participant_id: str, response: Response):
    filters = {
        "project_id": project_id,
        "experiment_id": experiment_id,
